...
CREATE TABLE `__new_audit_events` (
	...
	CONSTRAINT "audit_events_actor_check" CHECK(("__new_audit_events"."actor_kind" = 'staff' AND "__new_audit_events"."actor_id" IS NOT NULL AND "__new_audit_events"."staff_role" IS NOT NULL) OR ("__new_audit_events"."actor_kind" <> 'staff' AND "__new_audit_events"."staff_role" IS NULL)),
	CONSTRAINT "audit_events_actor_kind_check" CHECK("__new_audit_events"."actor_kind" IN ('system', 'staff', 'customer', 'provider')),
...
INSERT INTO `__new_audit_events`("id", "actor_kind", "actor_id", "staff_role", "source_channel", "action", "outcome", "entity_kind", "entity_id", "reason", "command_correlation_id", "metadata_json", "created_at") SELECT "id", "actor_kind", "actor_id", "staff_role", "source_channel", "action", "outcome", "entity_kind", "entity_id", "reason", "command_correlation_id", "metadata_json", "created_at" FROM `audit_events`;--> statement-breakpoint
