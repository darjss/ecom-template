...
CREATE TABLE `__new_orders` (
	...
	CONSTRAINT "orders_placement_key_check" CHECK(length("__new_orders"."placement_key") = 36),
	CONSTRAINT "orders_placement_digest_check" CHECK(length("__new_orders"."placement_intent_digest") = 64 AND "__new_orders"."placement_intent_digest" NOT GLOB '*[^0123456789abcdef]*'),
	...
	CONSTRAINT "orders_customer_link_check" CHECK(("__new_orders"."customer_id" IS NULL AND "__new_orders"."customer_linked_at" IS NULL) OR ("__new_orders"."customer_id" IS NOT NULL AND "__new_orders"."customer_linked_at" IS NOT NULL)),
	CONSTRAINT "orders_status_token_check" CHECK("__new_orders"."status_token_hash" IS NULL OR (length("__new_orders"."status_token_hash") = 64 AND "__new_orders"."status_token_hash" NOT GLOB '*[^0123456789abcdef]*')),
...
INSERT INTO `__new_orders`("id", "placement_key", "placement_intent_digest", "order_number", "state", "customer_id", "customer_linked_at", "status_token_hash", "recipient_name", "recipient_phone", "currency", "subtotal_mnt", "discount_total_mnt", "delivery_fee_mnt", "grand_total_mnt", "free_delivery_threshold_mnt", "free_delivery_applied", "commerce_settings_updated_at", "fulfillment_mode", "delivery_address", "pickup_location_id", "pickup_name", "pickup_address", "commercial_fingerprint", "placed_at", "created_at") SELECT orders."id", placement_idempotency."key", placement_idempotency."intent_digest", orders."order_number", orders."state", orders."customer_id", orders."customer_linked_at", orders."status_token_hash", orders."recipient_name", orders."recipient_phone", orders."currency", orders."subtotal_mnt", orders."discount_total_mnt", orders."delivery_fee_mnt", orders."grand_total_mnt", orders."free_delivery_threshold_mnt", orders."free_delivery_applied", orders."commerce_settings_updated_at", orders."fulfillment_mode", orders."delivery_address", orders."pickup_location_id", orders."pickup_name", orders."pickup_address", orders."commercial_fingerprint", orders."placed_at", orders."created_at" FROM `orders` INNER JOIN `placement_idempotency` ON `placement_idempotency`.`order_id` = `orders`.`id`;--> statement-breakpoint
DROP TABLE `placement_idempotency`;--> statement-breakpoint
...
CREATE INDEX `orders_phone_created_idx` ON `orders` (`recipient_phone`,`created_at`) WHERE "orders"."customer_id" IS NULL;