...
const normalizationBySource = new Map<string, string>();
const normalizationCandidates = [
  ...krilleerCyrillicToLatin.flatMap(([character]) =>
    [character, character.toUpperCase()].map((form) => form.normalize("NFD")),
  ),
  ...Array.from({ length: 10 }, (_, index) => String.fromCodePoint(65_296 + index)),
  ...Array.from({ length: 26 }, (_, index) => String.fromCodePoint(65_313 + index)),
  ...Array.from({ length: 26 }, (_, index) => String.fromCodePoint(65_345 + index)),
];
for (const source of normalizationCandidates) {
  const normalized = normalizeSearchText(source);
  if (normalized !== source) {
    normalizationBySource.set(source, normalized);
  }
}
const normalizationStages = Array.from(
  { length: Math.ceil(normalizationBySource.size / 4) },
  (_, index) => [...normalizationBySource.entries()].slice(index * 4, index * 4 + 4),
);
const normalizeSql = (field: string, replacements: (typeof normalizationStages)[number]) =>
  replacements.reduce(
    (expression, [source, normalized]) =>
      `replace(${expression}, ${quote(source)}, ${quote(normalized)})`,
    field,
  );
const normalizationCtes = normalizationStages
  .map((replacements, index) => {
    const name = `catalog_search_normalization_${index + 1}`;
    const source = index === 0 ? "catalog_search_source" : `catalog_search_normalization_${index}`;
    const projectedFields = transliterationFields
      .map((field) => `      ${normalizeSql(field, replacements)} AS ${field}`)
      .join(",\n");
    return `  ${name} AS (\n    SELECT\n      item_id, kind,\n${projectedFields}\n    FROM ${source}\n  )`;
  })
  .join(",\n");
const normalizationResult = `catalog_search_normalization_${normalizationStages.length}`;
...
const transliterationCtes = transliterationStages
  .map((mappings, index) => {
    ...
    const source = index === 0 ? normalizationResult : `catalog_search_transliteration_${index}`;
    const projectedFields = transliterationFields
      .map((field) => {
