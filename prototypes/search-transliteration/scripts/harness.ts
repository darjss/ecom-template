...
type Case = { label: string; query: string; expectedNames: string[]; expectedSource?: Source; expectedEmpty?: boolean; expectedBindings?: number };
...
for (const mode of ["strict", "basic", "tiered"] as const) {
  ...
  for (const item of cases) {
    ...
    if (mode === "tiered") {
      ...
      const expectedBindings = item.expectedBindings ?? 1;
      const bindingPass = result.data.timing_ms.binding_calls === expectedBindings;
