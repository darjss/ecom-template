...
Use native APIs for simple work and `es-toolkit` for real generic operations. Use specialized baseline dependencies only at their owning seams: `dismatch` for union matching, Culori for Theme colors, Micromark for constrained CMS Markdown, Solid Primitives for reactive persistence, and Ky for handwritten outbound HTTP. Ky is the sole handwritten outbound HTTP client; framework fetch handlers and transports remain framework interfaces. Treat external response bodies as unknown until Valibot parses them, and enable retries only when the owning business operation permits replay. Do not add a competing utility, validation, Result, state, date, icon, motion, logging, HTTP, or matching library without review.
