...
Every commerce identity, command, idempotency key, event, and reference is scoped to exactly one independently deployed Store, and all references resolve within it. Customer and Staff Member identities never link or merge across Stores, even when phone numbers match. Each store-local D1 is authoritative; no global commerce identity or cross-store aggregate exists in v1. The merchant app may choose presentation, but only the shared kernel may mutate price, inventory, checkout, Payment, Order, cancellation, refund, or Fulfillment truth.
...
Separate aggregates may change in one D1 transaction when an invariant spans them. Aggregate boundaries describe ownership and concurrency, not a requirement for separate network calls.
...
- Payment method is fixed at `PlaceOrder`. Same-method replacement attempts are allowed only while the original Active reservation remains valid and never extend its deadline. Switching method requires Cancellation and a new `PlaceOrder`.
- Failed, Expired, and Rejected are terminal for that Payment attempt. A QPay Failed attempt alone leaves the unpaid Order, Active reservation, and redemption claim until the original deadline so a same-method replacement remains possible. Explicit Cancellation, final deadline Expiry, or a kernel-classified final failure cancels the Order and compensates reservation and eligible redemption; adapters report evidence but never decide retryability or Order cancellation. QPay deadline expiry atomically expires Payment and reservation and cancels the unpaid Order. Manual-transfer or COD rejection atomically rejects Payment, releases the reservation, and cancels the unpaid Order. An Order may have multiple attempts but at most one active collectible Payment.
...
- A late provider success is recorded as Payment Evidence and a reconciliation exception. It never resurrects an expired reservation or confirm/cancel an Order without a kernel recovery command.
- Refund states record merchant-confirmed manual execution. Every Refund records integer-MNT amount, owner/manager actor, reason, external or manual reference when available, and explicit allocation across sold Order Lines and/or delivery fee. Allocations sum exactly to the Refund and cumulative allocations cannot exceed each component's net paid amount; Bundle allocation remains at the Bundle Order Line. Refund allocation never changes quantities or inventory. Partially Refunded may advance to Refunded; Refund records are immutable and corrected with compensating records.
...
A Variant maps to exactly one Stock Item in the store's single shared inventory pool. Available Quantity is current on-hand quantity minus all Active reservation demand and can never be negative. Bundle availability is derived from component Variant availability and required quantities; no Bundle stock is persisted. Placing an Order locks or conditionally revises every demanded Stock Item in a deterministic order so either all demand is reserved or none is.
...
Delivery Options are either free Pickup at an active public Location or Delivery priced by one storewide flat fee or a customer-selected merchant-defined Zone fee. Delivery becomes free when the post-discount merchandise subtotal meets the merchant's configured threshold. Checkout snapshots the mode, selected Location or Zone, submitted destination, fee, and threshold result. Customer Zone selection is submitted intent subject to merchant operational review, never geospatial proof. All Locations draw from the same inventory pool. Locations and Zones move `Draft → Active → Archived`; only Active choices may enter checkout. Revisioned edits affect future Orders only, while placed Orders retain immutable labels, addresses, fees, and destination facts. Archived is terminal in v1, preserves historical identity, and prevents identity reuse. Geospatial validation, distance pricing, and live courier quotes are excluded in v1.
...
Cancellation is a whole-Order decision, while Refund is a Payment fact. Line-level and quantity-level cancellation are excluded in v1; before handoff or Pickup, staff continue the complete Order or cancel it entirely. A Returned delivery permits narrow post-handoff Cancellation only after verified physical return; Delivery Failed alone never permits cancellation or stock restoration. Unconfirmed COD becomes Rejected; any Confirmed Payment—including COD—preserves immutable confirmation truth and creates a Refund Obligation. Confirmed never transitions to Rejected. Returns after Fulfilled remain outside v1. Discretionary partial Refunds remain financial-only and never alter ordered quantities, Inventory Demand, pricing, or Fulfillment. Cancellation and Refund are related but never aliases:
...
- A Refund Obligation progresses `Open → Partially Satisfied → Satisfied` as owner/manager records immutable manual Refunds. It cannot be silently deleted or waived; corrections require audited compensating financial evidence.
- Owner/manager may record a mandatory-reason, explicitly allocated partial or full Refund against any Confirmed Payment without cancelling the Order. It satisfies an existing Refund Obligation when present; otherwise it is an audited discretionary financial action. It never changes Order, Fulfillment, quantities, or inventory automatically.
...
- D1 tables, constraints, indexes, TypeID prefixes, and transaction mechanics;
