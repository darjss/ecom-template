...
A Variant maps to exactly one Stock Item in the store's single shared inventory pool. Available Quantity is current on-hand quantity minus all Active reservation demand and can never be negative. Bundle availability is derived from component Variant availability and required quantities; no Bundle stock is persisted. The minimum reservation model is one D1-backed Inventory Reservation per Order, normalized Variant demand rows, and Stock Item on-hand and reserved balances. Placing an Order conditionally updates every demanded Stock Item in one D1 transaction so either all demand is reserved or none is. Inventory reservation does not use Durable Objects; keeping Order, Payment, discount, and inventory changes in the store-local D1 preserves one transaction boundary. Warehouses, stock batches, partial reservations, waitlists, and automatic rebalancing are excluded in v1.
