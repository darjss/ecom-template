...
Logs never include names, phone numbers, email or delivery addresses, free-text notes, OTPs, cookies, authorization headers, tokens, QR payloads, bank details, full provider payloads, raw request bodies, or PostHog distinct IDs. Expected validation and authorization rejections are not logged as infrastructure errors. Payment, inventory, and staff changes remain authoritative in D1 entries and Audit Events; logs are short-lived diagnostic evidence, not a ledger.
