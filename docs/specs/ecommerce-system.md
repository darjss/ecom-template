...
- Staff sessions are revocable rolling 14-day sessions using a role snapshot. Role change or revocation deletes all sessions. Every authorized Staff request also verifies that the session identity and role still match the active Staff record, so a session created during an authority-change race cannot retain stale access.
