...
- Public API includes private/no-store Catalog search and `POST /api/catalog/availability`. Availability accepts a strict body containing `targets`: a bounded array of 1–50 unique discriminated purchase targets, each either `{ kind: "variant", id: <canonical Variant ID>, quantity: 1..999 }` or `{ kind: "bundle", id: <canonical Bundle ID>, quantity: 1..999 }`. It returns only requested purchase identity/kind, advisory sellability, current positive integer-MNT unit price, and one server `checkedAt`; unknown or unpublished targets are omitted and raw inventory balances are never exposed. Bundle sellability expands complete component Variant demand for the requested Bundle quantity without a Bundle stock record.
- Checkout provides current quote and the narrow QPay-to-bank-transfer switch. A repeated placement request may create another Order; the UI prevents accidental duplicate submission.
...
- Staff sessions are revocable rolling 14-day sessions using a role snapshot. Approval, role change, revocation, and removal delete all sessions before mutation and again after the committed target state. Normal authorization does not require a redundant D1 role read on every request.
